...
class ModuleBuilder:
    ...
    def match_node(self, node: CallableAst, name: str, obj_id: int) -> None:
        match node:
            case ast.Lambda():
                self._add_lambda_def(node, name, obj_id)

            case ast.FunctionDef():
                self._add_func_def(node, name, obj_id)

    def _add_func_def(self, node: ast.FunctionDef, name: str, obj_id: int) -> None:
        node.name = name
        module_node = ast.Module(body=[node], type_ignores=[])
        self.definitions[obj_id] = ast.unparse(module_node)

    def _add_lambda_def(self, node: ast.Lambda, name: str, obj_id: int):
    ...
    for name, obj in scope.items():
        ...
        if isinstance(obj, Func):
            ...
            if func_def := finder.found_func:
                alias = f"{name} = {func_def.name}"
                builder.definitions[obj_id] = f"{obj.source_code}\n{alias}"
